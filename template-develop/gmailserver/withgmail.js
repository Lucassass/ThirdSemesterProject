...
server.post('/forgotPass', async function (req, res) {
    ...
    if (validator.isEmail(endEmail)) {
        ...
        if (bool === 'true') {
            ...
            if (bool) {
                res.send({ msg: 'Email is send', isSent: true, params: encrypted });
            } else {
                res.sendStatus(500);
            }
        } else {
...
server.post('/resetPassword_form', async function (req, res) {
    ...
    console.log('mailtoken: ' + encryptedMail);
    ...
    if (sucess) {
        ...
        res.status(201).json({ msg: 'the users password has been reset', success: true });
    } else {
        res.json({ msg: 'Not a valid user', success: false });
    }
});
