...
app.post('/refresh', authenticateRefreshToken, (req, res) => {
    ...
    try {
        //få fat i userId og subType
        getUserId(req.decodedRefreshToken.email).then((userId) => {
            getSubscriptionType(userId).then((subType) => {
                const userPayload = {
                    email: req.decodedRefreshToken.email,
                    subType: subType,
                    userId: userId
                };
...
//logs in the user with email and password. return a new access- and refresh token
        ...
        login(email, password).then((isAuth) => {
            if (isAuth) {
                getUserId(email).then((userId) => {
                    ...
                    //get userId, email, subscription mode(int), admin (bool)
                    getSubscriptionType(userId).then((subType) => {
                ...
                //provided email + password did not match any users in DB
...
app.post('/getUserAgents', authenticateRefreshToken, (req, res) => {
    getUserAgentsAndRefreshId(req.decodedRefreshToken.email).then((agentList) => {
...
function authenticateRefreshToken(req, res, next) {
    ...
    //verifies signature of refresh token and returns the payload (refresh id and email)
...
function getSubscriptionType(userId) {
    return fetch(subscriptionURL + '/customer/' + userId + '/get_subscription_type', {
        ...
        .catch((error) => {
            ...
            throw 'could not fetch endpoint /customer/:id/get_subscription_type: error occurred with the subscription data API';
        });
}
/**
 *
...
function getUserId(email) {
    return fetch(dataSecurityURL + '/getUserId', {
        ...
        .catch((error) => {
            ...
            throw 'could not fetch endpoint /getUserId: error occurred with the data secuirty API';
        });
}
/**
 *
...
function storeRefreshId(email, refreshId, userAgent) {
    return fetch(dataSecurityURL + '/refresh', {
        ...
        .catch((error) => {
            ...
            throw 'could not fetch endpoint /refresh: error occurred with the data secuirty API';
        });
}
/**
 *
...
function verifyRefreshId(refreshId) {
    return fetch(dataSecurityURL + '/checkrefresh', {
        ...
        .catch((error) => {
            ...
            throw 'could not connect fetch endpoint /getUserId: error occurred with the data secuirty API';
        });
}
/**
 *
...
function removeRefreshId(refreshId) {
    return fetch(dataSecurityURL + '/logout', {
        ...
        .catch((error) => {
            ...
            throw 'could not connect fetch endpoint /logout: error occurred with the data secuirty API';
        });
}

/**
 * gives a email and recieves all user-agents + refreshId, så that a user could log out other divivs currently logged in
 * @param {*} email //data security expect the body to contain a field with key "email", although in reality it is a email
 */
function getUserAgentsAndRefreshId(email) {
    return fetch(dataSecurityURL + '/getUserAgents', {
        ...
        .catch((error) => {
            ...
            throw 'could not fetch endpoint /getUserAgents: error occurred with the data secuirty API';
        });
}
